...
struct OnboardingViewPure: View {
    ...
    func nextButton() {
        if self.curSlideIndex == self.onboardData.count - 1 {
            doneFunction()
            return
        }
    ...
    var body: some View {
        ZStack {
            ...
            ZStack(alignment: .center) {
                ForEach(0..<onboardData.count) { i in
                    OnboardingStepView(data: self.onboardData[i])
...
struct OnboardingViewPure_Previews: PreviewProvider {
    ...
    static var previews: some View {
        OnboardingViewPure(onboardData: sample, doneFunction: { print("done") })
    }
}
